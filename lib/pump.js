...
// On-chain sharing config and social vault PDA structures from the pfee program IDL.
...
// Social vault PDA seeds: ["social-fee-pda", numeric_user_id_string, [platform_byte]]
// Platform enum (from IDL): Pump=0, X=1, GitHub=2, Google=3, Discord=4,
//   Reddit=5, TikTok=6, YouTube=7, Twitch=8, LinkedIn=9, Facebook=10,
//   Instagram=11, Snapchat=12, Telegram=13
...
// Sharing-config layout: disc(8) + ?(32) + admin(32) + flags(4) + vec_len(4) + shares(n×36)
//   share: recipient_pubkey(32) + bps_u32(4)
const BC_CONFIG_OFFSET = 49;

const HEADERS = {
  'User-Agent': 'pumpfun-creator-fees/0.1 (+https://github.com/nirholas/pumpfun-creator-rewards)',
...
// ── Solana PDA derivation (pure Node, no @solana/web3.js) ────────────────────
...
// Returns true if bytes represent a valid compressed ed25519 point (i.e. ON the curve).
// A valid Solana PDA must be OFF the curve.
function isOnEd25519Curve(bytes) {
  let y = 0n;
  for (let i = 0; i < 32; i++) y |= BigInt(bytes[i]) << BigInt(8 * i);
  y &= (1n << 255n) - 1n; // strip sign bit
  const y2 = y * y % ED25519_P;
  const u = modP(y2 - 1n);
  const v = modP(ED25519_D * y2 + 1n);
  const vInv = powModP(v, ED25519_P - 2n);
  const x2 = u * vInv % ED25519_P;
  if (x2 === 0n) return true;
  // check square root exists: x2^((p-1)/2) must be 1
  return powModP(x2, (ED25519_P - 1n) / 2n) === 1n;
}

function findProgramAddressSync(seeds, programIdStr) {
  const progBytes = Buffer.from(b58dec(programIdStr));
  const pda = Buffer.from('ProgramDerivedAddress');
  for (let bump = 255; bump >= 0; bump--) {
    const data = Buffer.concat([...seeds, Buffer.from([bump]), progBytes, pda]);
    const hash = createHash('sha256').update(data).digest();
    if (!isOnEd25519Curve(hash)) return { address: b58enc(hash), bump };
  }
  throw new Error('could not find PDA');
}

// Derive the social vault PDA for a given platform + numeric user ID string.
...
async function resolveXId(username) {
  // Pump.fun stores numeric Twitter/X user IDs.
  // The public API requires auth; try a free proxy approach.
  // If unavailable, caller should surface a helpful message.
  try {
    const res = await fetch(`https://api.twitter.com/2/users/by/username/${encodeURIComponent(username)}`, {
      headers: { 'User-Agent': 'pumpfun-creator-fees/0.1' },
    });
    if (!res.ok) return null;
    const data = await res.json();
...
// ── Platform-prefixed input parsing ──────────────────────────────────────────
// Supported formats:
//   github:username   gh:username
//   x:username        twitter:username
//   tiktok:username   tt:username
//   discord:username
//   reddit:username
//   pump:username     (pump.fun username)
//   <base58>          wallet or mint
//   bare string       pump.fun username fallback
...
async function resolveSocialVault(platform, username) {
  ...
  if (platform === 'github') {
    ...
    if (!numericId) {
      throw new Error(`X/Twitter numeric ID for @${username} couldn't be resolved automatically. Paste their numeric Twitter ID after "x:" (e.g. x:123456789).`);
    }
  } else if (platform === 'pump') {
    // Pump.fun username — use existing user lookup
    ...
    // For other platforms we need the numeric ID from the user
    // Try treating the username directly as a numeric ID first
    if (/^\d+$/.test(username)) {
      numericId = username;
    } else {
      throw new Error(`For ${platform}, please provide the numeric user ID (e.g. ${platform}:12345678). Username-to-ID lookup isn't available for this platform yet.`);
    }
  }
...
export async function resolveInput(raw) {
  ...
  // Platform-prefixed: github:nirholas, x:elonmusk, tiktok:123456, etc.
  const prefixed = parsePlatformPrefix(q);
  if (prefixed) {
    return resolveSocialVault(prefixed.platform, prefixed.username);
  }

  // Base58 — could be mint or wallet
  if (BASE58_RE.test(q)) {
  ...
  // Bare string — try pump.fun username
  const user = await fetchUserByUsernameOrAddress(q);
  if (user?.address) {
    return { type: 'username', wallet: user.address, username: user.username || q, userMeta: user };
  }

  throw new Error(
    `"${q}" not found. Try:\n• github:username\n• x:numeric_id (e.g. x:44196397)\n• tiktok:numeric_id\n• A coin mint or wallet address\n• A pump.fun username`
  );
}
